<template>
  <div class="page-container">
    <el-card>
      <template #header>
        <div class="card-header">
          <span>成品库存 (Finished Goods Inventory)</span>
      ...
      <el-form :inline="true" :model="filters" class="filter-form">
        <el-form-item label="产品筛选">
          ...
          >
            <el-option
              v-for="material in finishedGoods"
              :key="material.id"
      ...
      >
        <el-table-column prop="material.code" label="产品编码" width="120" />
        <el-table-column prop="material.name" label="产品名称" width="200" />
    ...
    <el-dialog
      ...
      :title="`${selectedMaterialName} - 成品流水`"
      width="1200px"
...
const loadInventory = async () => {
  try {
    ...
    // Filter to only show finished goods
    if (!filters.materialId) {
      const finishedGoodIds = finishedGoods.value.map(m => m.id)
      inventoryData.value = response.data.filter((inv: any) =>
        finishedGoodIds.includes(inv.material?.id)
      )
...
const viewTransactionHistory = (materialId: number) => {
  ...
  const material = finishedGoods.value.find((m) => m.id === materialId)
  selectedMaterialName.value = material ? `${material.name} (${material.code || material.id})` : '未知产品'
...
const getTransactionTagType = (type: string): string => {
  const inboundTypes = ['ProductionIn', 'OutsourceIn', 'ReturnIn', 'AdjustmentIn']
